-- Corrected Test Case Population - Using Actual Table Names
...
-- Populate message_definitions with 500+ entries (this is the correct table name)
DO $$
DECLARE
  i INTEGER;
  message_id UUID;
...
BEGIN
  ...
  FOR i IN 1..500 LOOP
    ...
    -- Insert message definition (correct table name)
...
-- Populate ie_definitions with 1000+ entries (this is the correct table name)
DO $$
DECLARE
  i INTEGER;
  ie_id UUID;
...
BEGIN
  ...
  FOR i IN 1..1000 LOOP
    ...
    -- Insert information element definition (correct table name)
...
CREATE INDEX IF NOT EXISTS idx_message_definitions_protocol_spec_id ON message_definitions(protocol_spec_id);
CREATE INDEX IF NOT EXISTS idx_ie_definitions_protocol_spec_id ON ie_definitions(protocol_spec_id);
